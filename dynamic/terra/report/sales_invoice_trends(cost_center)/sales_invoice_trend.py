...
def get_data(filters, conditions):
	...
	if filters.get("group_by"):
		...
		# frappe.errprint(f'\n\n->>conditions["columns"]-->{conditions["columns"]}')
		# frappe.errprint(f'\n\n->>conditions["grbc"]-->{conditions["grbc"]}')
		# frappe.errprint(f'\n\n->>ind-->{ind}')
		...
		data1 = frappe.db.sql(
			...
			% (
				...
				conditions["trans"],
				conditions["trans"],
				conditions["addl_tables"], #none
				"%s",
				posting_date,
				"%s",
				"%s",
				conditions.get("addl_tables_relational_cond"),#none
				cond,
				conditions["group_by"],
			),
...
def based_wise_columns_query(based_on, trans):
	...
	if based_on == "Item":
		based_on_details["based_on_cols"] = ["Item:Link/Item:120", "Item Name:Data:120","Cost Center:Link/Cost Center:120"]
		based_on_details["based_on_select"] = "t2.item_code, t2.item_name,t2.cost_center,"
