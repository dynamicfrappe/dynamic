...
frappe.ui.form.on('Real State Cost', {
 ...
 erpnext.stock.LandedCostVoucher = erpnext.stock.StockController.extend({
	setup: function() {
		...
		// this.frm.add_fetch("receipt_document", 'total_incoming_value', "grand_total"); //total_outgoing_value
...
frappe.ui.form.on('Landed Cost Taxes and Charges', {
	...
	amount: function(frm, cdt, cdn) {
		...
		frappe.model.set_value(cdt, cdn, "base_amount",
			flt(flt(row.amount)*(row.exchange_rate > 0 ? row.exchange_rate : 1), precision("base_amount", row)));
	},
	
});


frappe.ui.form.on("Landed Cost Stock Entry", {
			...
			frappe.call({
				...
				callback(r) {
					...
					if (r.message) {
						...
						else if(r.message.total_outgoing_value>0){
							v_grand_total = r.message.total_outgoing_value
						}
						row.grand_total=v_grand_total
