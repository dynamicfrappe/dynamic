...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		...
		frappe.errprint(f"abbr_or_value-->{abbr_or_value}")
		abbreviations.append(abbr_or_value)
	
	if abbreviations:
		frappe.errprint(f"abbreviations-->{abbreviations}")
		frappe.throw("test")
		# variant.item_code = "{0}-{1}".format(template_item_code , abbreviations[0])
		# variant.item_name = "{0}-{1}".format(template_item_code,  abbreviations[0])
		# variant.description = "{0}-{1}".format(template_item_code , "-".join(abbreviations))
