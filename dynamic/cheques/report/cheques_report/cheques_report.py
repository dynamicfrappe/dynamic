...
class Cheques_report(object):
    ...
    def get_data(self):
        ...
        self.data = self.get_data_from_payment_entry(
    ...
    def get_data_from_payment_entry(self, conditions='', values=''):
        query_test_p = """
		select p.name as `Payment`,p.reference_no as `Cheques NO`,p.party as `Party`,p.party_type as `Party Type`,p.cheque_status as `Cheque Status`, p.paid_amount as `Amount`,p.posting_date as `Transaction Date`,p.reference_date as `Reference Date`,p.drawn_bank as `Bank`
    ...
    def get_conditions(self, filters):
        ...
        if filters.get("bank_account"):
            conditions += " AND p.bank_account =  %(bank_account)s "
