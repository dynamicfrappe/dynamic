...
def execute(filters=None):
	...
	frappe.errprint(f'item_details==>{item_details}')
	data = []
	conversion_factors = []
	if opening_row:
...
def get_columns():
	columns = [
		...
		{"label": _("Item Price"), "fieldname": "item_price", "width": 100},
		{
			"label": _("Stock UOM"),
			"fieldname": "stock_uom",
			"fieldtype": "Link",
...
def get_item_details(items, sl_entries, include_uom,filters):
	...
	pl_join = (
			"LEFT JOIN `tabItem Price` ON `tabItem Price`.item_code=item.name AND `tabItem Price`.selling=1 AND `tabItem Price`.price_list='%s'"%(filters.get('price_list'))
		)
	...
	""".format(
			cf_field=cf_field, cf_join=cf_join,pl_field=pl_field,pl_join=pl_join, item_codes=",".join(["%s"] * len(items)
			)
		),
		items,
		as_dict=1,
