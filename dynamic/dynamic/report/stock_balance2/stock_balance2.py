...
def execute(filters=None):
	...
	for i in data:
		i['total_weight_balance'] = i['bal_qty'] * i['weight_per_unit']
	return columns, data
...
def get_columns(filters):
	...
	columns = [
		...
		{
			"label": _("Total Weight Balance"),
			"fieldname": "total_weight_balance",
			"fieldtype": "Float",
			"width": 90,
		},
		{
...
def get_item_details(items, sle, filters):
	...
	res = frappe.db.sql(
		"""
		select
			item.name, item.item_name, item.description, item.item_group, item.brand, item.weight_per_unit , item.stock_uom %s
		from
			`tabItem` item
			%s
		where
