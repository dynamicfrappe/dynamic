...
def get_data(
	...
	if not accounts:
		return None
	#print("account from old funcions",accounts)
...
def get_cost_of_good_sold_accounts(company, account):
	accoun_obj = frappe.get_doc("Account",account)
	if accoun_obj.is_group:
		return frappe.db.sql(
			"""
			select name, account_number, parent_account, lft, rgt, root_type, report_type, account_name, include_in_gross, account_type, is_group, lft, rgt
			from `tabAccount`
			where company=%s and (parent_account=%s or name=%s) order by lft""",
		...
		return frappe.db.sql(
			...
			select name, account_number, parent_account, lft, rgt, root_type, report_type, account_name, include_in_gross, account_type, is_group, lft, rgt
			from `tabAccount`
			where company=%s and name=%s order by lft""",
			(company, account),
			as_dict=True,
		)
...
def filter_cost_accounts(accounts, depth=20):
	parent_children_map = {}
	accounts_by_name = {}
	for d in accounts:
		accounts_by_name[d.name] = d
...
def set_gl_entries_by_account( 
	...
	if accounts:
		...
		distributed_cost_center_query = f"""
			UNION ALL
			SELECT posting_date,
				account,
			...
			(
				...
				WHERE cost_center  = '{cost_center_1}'
				AND parent NOT  = '{cost_center_1}'
				GROUP BY parent
			) as DCC_allocation
			...
			""".format(
				additional_conditions=additional_conditions.replace(f"and cost_center = '{cost_center_1}'", "")
			)

		gl_entries = frappe.db.sql(
			f"""select posting_date, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency from `tabGL Entry`
			where company=%(company)s
			{additional_conditions}
			and posting_date <= %(to_date)s
			and is_cancelled = 0 and cost_center = '{cost_center_1}'
		""",
			gl_filters,
			as_dict=True,
		)  # nosec
