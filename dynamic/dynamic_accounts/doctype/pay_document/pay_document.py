...
class PayDocument(Document):
	...
	def validate(self):
		...
		if self.difference != 0 :
			frappe.msgprint(_("Total Difference is {}").format(self.difference))
	...
	def create_journal_entry (self):
		...
		je.remark = f'Payment against {self.doctype}: ' + self.name + '\n' + (getattr(self,'notes',"") or "")
		...
		if account_currency != self.currency :
			if account_currency == company_currency :
				...
				account_exchange_rate = get_exchange_rate(account_currency, company_currency, self.posting_date)
				amount_in_account_currency = flt(self.amount) * flt(account_exchange_rate)
		...
		je.append("accounts", {
			...
			"user_remark" : getattr(self,'notes',""),
		...
		for account_row in self.accounts :
			...
			if account_row_currency != account_row.currency :
				if account_row_currency == company_currency :
						...
						amount_in_account_row_currency = flt(account_row.base_amount)

				else :
					account_row_exchange_rate = get_exchange_rate(account_currency, company_currency, self.posting_date)
					amount_in_account_row_currency = flt(account_row.amount) * flt(account_row_exchange_rate)
			...
			je.append("accounts", {
				...
				"reference_type" : self.doctype,
				"user_remark" : getattr(account_row,"notes",""),
				"reference_name" : self.name,
