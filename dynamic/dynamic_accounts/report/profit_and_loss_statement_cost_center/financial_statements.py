...
def get_data(
	...
	# frappe.errprint(f'accounts_by_name=+---+=>{accounts_by_name}')
			...
			for period in period_list:
				...
				if entry.posting_date <= period.to_date:
					if (accumulated_values or entry.posting_date >= period.from_date) and (
						...
						# frappe.errprint(f'd in if ==>{d}')
			if entry.posting_date < period_list[0].year_start_date:
				d["opening_balance"] = d.get("opening_balance", 0.0) + flt(entry.debit) - flt(entry.credit)
				# frappe.errprint(f'zzzzd in if ==>{d}')
	...
	for d in reversed(accounts):
		if d.parent_account:
			for period in period_list:
				...
				# frappe.errprint(f'perid key ---> {period.key}')
				# frappe.errprint(f'd.parent_account ---> {d.parent_account}')
...
def set_gl_entries_by_account(
	...
	# frappe.throw(f"root type ,{root_type}")
	accounts = frappe.db.sql_list(
		"""select name from `tabAccount`
		where lft >= %s and rgt <= %s and company = %s and root_type = %s """,
		(root_lft, root_rgt, company ,root_type),
	)
	...
	if accounts:
		...
		if filters and filters.get("cost_center"):
			...
			gl_entries = frappe.db.sql(
				"""select cost_center,posting_date, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency from `tabGL Entry`
				where company=%(company)s
				{additional_conditions}
				and posting_date <= %(to_date)s
				and is_cancelled = 0
			...
			# frappe.errprint(f'gl_entries=++=>{gl_entries}')

			for entry in gl_entries:
				gl_entries_by_account.setdefault(entry.account, []).append(entry)

			gl_entries_by_cost_center={}
			for entry in gl_entries:
				gl_entries_by_cost_center.setdefault(entry.cost_center, []).append(entry)

		frappe.errprint(f'gl_entries_by_cost_center==>{gl_entries_by_cost_center}') 
		# frappe.errprint(f'gl_entries_by_cost_center==>{gl_entries_by_cost_center}') 

		return gl_entries_by_cost_center #gl_entries_by_account
